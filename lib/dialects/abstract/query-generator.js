...
class QueryGenerator {
  ...
  */
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
    ...
    if (include.subQuery && topLevelInfo.subQuery) {
      ...
      if (requiredMismatch && mainChildIncludes.length > 0) {
        joinQueries.mainQuery.push(` ${joinQuery.join} ( ${joinQuery.body}${joinQuery.indexHints ?? ''}${mainChildIncludes.join('')} ) ON ${joinQuery.condition}`);
      } else {
        joinQueries.mainQuery.push(` ${joinQuery.join} ${joinQuery.body}${joinQuery.indexHints ?? ''} ON ${joinQuery.condition}`);
  ...
  generateJoin(include, topLevelInfo) {
    ...
    let indexHints = '';
    if (include.indexHints && this._dialect.supports.indexHints) {
      for (const hint of include.indexHints) {
        if (IndexHints[hint.type]) {
          indexHints += ` ${IndexHints[hint.type]} INDEX FOR JOIN (${hint.values.map(indexName => this.quoteIdentifiers(indexName)).join(',')})`;
        }
      }
    }
