  ...
  var validateIncludedElement = function(include, tableNames) {
    ...
    if (association) {
      ...
      if (Object(include.association.through) === include.association.through) {
        ...
        if (!through instanceof DAOFactory)
        {
          if (typeof through.model === 'undefined') {
            throw new Error('Include.through malformed. Expected attribute: model!')
          }

          extra = Utils._.pick(through, ['where', 'attributes'])
          through = through.model
        }

        include.through = Utils._.assign({
          daoFactory: through,
